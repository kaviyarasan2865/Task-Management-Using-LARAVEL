                ...
                <tbody>
                    @foreach($tasks as $task)
                        <tr>
                            ...
                            <td>{{ $task->status }}</td>
                            <td>
                                <button type="button" class="btn btn-primary updateStatusBtn" data-task-id="{{ $task->id }}">Update Status</button>
                                <form method="post" action="{{ route('tasks.delete', ['id' => $task->id]) }}" style="display: inline;">
                                    @csrf
                                    @method('DELETE')
...
{{--             <a href="{{ route('tasks.create.form') }}" class="btn btn-success">Add New Task</a>
 --}}
...
<script>
    $(document).ready(function() {
        $('.updateStatusBtn').click(function() {
            ...
            $.ajax({
                url: "{{ route('tasks.showUpdateForm', ['id' => ':id']) }}".replace(':id', taskId),
                type: 'GET',
